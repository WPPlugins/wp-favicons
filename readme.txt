...
* Note 1: this plugin only supports PHP version 5.3 or later.
* Note 2: this plugin is still in BETA until it is at version 1.0
* Note 3: version 0.6.0 is the first one to split server and client functionality so may be atill a bit rough...
...
* includes a server component and a client component, so you can have a single icon server on 
  ...
  and the server if your run this on just 1 installation, I have found however that it is much 
  handier to centralize the server on 1 wp install since then on only that install it will 
  create a database and have a unique list of icons over multiple clients. 
* add icons in content, widget areas, comment area and comment author link each with its own styling.
* add icons from scanning sites directly, their /favicon.ico, Google 2 and getIcon.org
* image filtes such as converting all icons to PNG
* add default icons, IDENTICON is included
...
hour (and even then only if a visitor hits that page). So it will build up very slowly with
performance and being nice to your database and providers. However notice that if you have
...
of changes full translation will happen in a later release.
